...
export const useExamImport = (params: {
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      ...
      const imageObjToDataUrl = (img: PdfImageObj): string | null => {
        ...
        try {
          if (kind === 3) {
            ...
            // kind=1: GRAYSCALE_1BPP — each byte = 8 pixels
            rgba = new Uint8ClampedArray(width * height * 4);
            for (let j = 0; j < width * height; j++) {
      ...
      // IMAGE_OPS: paintImageMaskXObject=83, paintImageXObject=85, paintInlineImageXObject=86, paintImageXObjectRepeat=88
      ...
      for (let pageNum = 1; pageNum <= pdf.numPages; pageNum += 1) {
        ...
        for (const name of imgNames) {
          const imgObj = await new Promise<PdfImageObj | null>((resolve) => {
            try { page.objs.get(name, (obj) => resolve(obj ?? null)); }
            catch { resolve(null); }
          });
